...
void update_range_value_util(int *st, int ss, int se, int qs,int qe,int val, int si)
{
	// Base Case: If the input index lies outside the range of this segment
	// during initial call, if it lies outside the whole segment tree's range index
    	if (qe < ss || qs > se || qs > qe)
        {
		return;
 	}
    	...
    	if(ss==se)
	{
		...
		return;
       	...
       	int mid = get_mid(ss, se);
       	update_range_value_util(st, ss, mid, qs, qe, val, 2*si + 1);
        update_range_value_util(st, mid+1, se, qs, qe, val, 2*si + 2);
	st[si]=st[2*si+1]+st[2*si+2];
	
}
 
// The function to update a range's value by in val input array and segment tree.
// why val? instead of increment_val or decrement_val. Because, we coudl do both
// It uses update_range_value_util() to update the value in segment tree
void update_range_value(int arr[], int *st, int n, int qs,int qe,int val)
{
	// Check for erroneous input index
    	if (qs < 0 || qe > n-1 || qs > qe)
    	{
        	printf("Invalid range for range update operation");
        	return;
    	}
	//update the values in actual array
	for(int i=qs;i<=qe;i++)
	{
		arr[i]+=val;
	}
    	// Update the values of nodes in segment tree
    	update_range_value_util(st, 0, n-1,qs,qe,val, 0);
}
	...
	try
		...
		st = construct_st(arr, n);
 		// Print sum of values in array from index 1 to 3
    		...
    		// segment tree nodes
   		 update_range_value(arr, st, n, 1, 3, 10);
